...
import './forge-bucket-storage.js';
import './forge-scroll-pane.js';
import './forge-card.js';
import { getModuleUrl  } from '../tricore-util/tricore-util.js';
// Extend Polymer.Element base class
class ForgeBucket extends PolymerElement 
{
  static get template() {
    return html`
        <style>
            ...
            <app-toolbar>
                ...
                <div style="float:right; display:inline-flex">
                    <paper-dropdown-menu label="Region">
                        <paper-listbox id="newBucketRegion" slot="dropdown-content" attr-for-selected="value" selected="{{region}}">
                                        ...
                                        <tr>
                                            ...
                                            <td>
                                                <template is="dom-if" if="[[!item.hidden]]">
                                                      <paper-icon-button icon="delete-forever" on-click="revoke" alt="Revoke rights" class\$="icon"></paper-icon-button>
                                                </template>
                                            </td>
                                        </tr>
                                    </template>
        ...
        </forge-scroll-pane></app-header>
        

        <paper-dialog id="newBucketDlg">
            <h2>Add Bucket</h2>
            <paper-dialog-scrollable>
                <paper-input id="newBucketKey" floatinglabel="" label="Key" pattern="[-_.a-z0-9]{3,128}" auto-validate=""> 
                </paper-input>
                <p>
        ...
        <forge-bucket-storage id="storage" appkey="{{appKey}}" restpath="[[restpath]]" on-bucket-list="_onBucketList" on-bucket-add="_onBucketAdd" on-bucket-delete="_onBucketDelete" on-bucket-detail="_onBucketDetail" on-bucket-grant="_onGrant" on-bucket-revoke="_onRevoke">
...
`;
  ...
  static get importMeta() {
    return getModuleUrl("tri-forge-service/forge-bucket.js");
  }
}
