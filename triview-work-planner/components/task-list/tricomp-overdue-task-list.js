...
class TricompOverdueTaskList extends TrimixinTaskList(PolymerElement) {
	static get is() { return "tricomp-overdue-task-list"; }
	...
	static get template() {
		return html`
			...
			<triservice-work-task overdue-tasks="{{_nonFilteredTasks}}" loading-overdue-tasks="{{_loading}}"></triservice-work-task>
					...
					<iron-list id="taskIronList" class="iron-list" as="task" items="[[_filteredTasks]]">
						<template>
							<tricomp-task-card class="task-card"
								...
								assignment-status-field="[[!smallLayout]]"
								assign-button="[[_computeShowAssignButton(smallLayout, _canAssign, task.assignmentStatusENUS)]]"
								duo-on-field="[[_isAssigned(task.assignmentStatusENUS)]]"
								type-field-middle="[[_isAssigned(task.assignmentStatusENUS)]]"
								planned-start-end-field="[[_isUnassigned(task.assignmentStatusENUS)]]"
								time-estimate-field="[[_isUnassigned(task.assignmentStatusENUS)]]"
								type-field-last="[[_isUnassigned(task.assignmentStatusENUS)]]"
	...
	static get properties() {
		return {
			_noTasksMessage: {
				type: String,
				value: () => {
					let __dictionary__message =  "No overdue work tasks are planned to start in the selected week for the selected workgroup.";
	...
	_handleSelectedTasksChange(selectedTasksChange, active) {
		if (active) {
			...
			if (selectedTasks && selectedTasks.length == 1 && selectedTasks[0].assignmentStatusENUS == "Assigned") {
	...
	_isUnassigned(status) {
		return status && status == "Unassigned";
	}

	static get importMeta() {
		return getModuleUrl("triview-work-planner/components/task-list/tricomp-overdue-task-list.js");
	}
}

window.customElements.define(TricompOverdueTaskList.is, TricompOverdueTaskList);